...
function fastifyObjectionjs(fastify, options, next) {
  const knexConfig = Object.assign(
    ...
    knexSnakeCaseMappers({
      ...
      underscoreBetweenUppercaseLetters:
        options.underscoreBetweenUppercaseLetters || false,
    })
  );
  ...
  if (supportedClients.indexOf(knexConfig.client) === -1) {
    next(
      new Error(
        `unsupported client, 'fastify-objectionjs' only support ${supportedClients.join(
          ", "
        )}.`
      )
    );
  ...
  if (options.models) {
    if (!Array.isArray(options.models) || options.models.length < 1) {
      next(
        new Error("You need to provide a valid array of `objection.js` models.")
      );
      return;
    }
    ...
    for (let i = 0; i < options.models.length; i += 1) {
      ...
      if (model.idColumn && model.tableName && model.QueryBuilder) {
        objection.models[model.name.replace(/^\w/, (c) => c.toLowerCase())] =
          model;
      }
    }
